...
export class WechatService {
  ...
  public async getAuthorizationCodeMiniApp(
    appNamespace: string,
    code: string,
  ): Promise<WxAuthorizationCode> {
  ...
  public async getAuthorizationCodeOAuth(
    appNamespace: string,
    code: string,
  ): Promise<WxAuthorizationCode> {
    const { appId, appSecret } = await this.getAppIdAndSecret(appNamespace);
  ...
  public async getWxACodeUnlimited(
    appNamespace: string,
    data: WxACodeUnlimitedDto,
  ): Promise<Buffer> {
  ...
  public async uploadImage(
    ...
    if (imagePath.startsWith('s3://')) {
      try {
        ...
        const response = await fetch(
          `${WECHAT_API_ROOT}/cgi-bin/media/upload?type=image&access_token=${access_token}`,
          {
  ...
  public async decodeEncryptedXmlMessage(
    appNamespace: string,
    encrypted: string,
  ): Promise<any> {
  ...
  public async validateWechatServer(
    appNamespace: string,
    signature: string,
    timestamp: string,
    nonce: string,
